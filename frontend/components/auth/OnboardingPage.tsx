...
const OnboardingPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await api.onboardCompany({ companyName, adminName, adminEmail });
            if (response.success) {
                setSuccessMessage(response.onboarding?.message || response.message || 'Workspace created successfully.');
            } else {
