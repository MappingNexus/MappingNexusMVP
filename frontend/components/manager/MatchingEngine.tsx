...
const MatchingEngine: React.FC = () => {
 ...
 <div><h1 className="text-2xl font-black text-foreground uppercase tracking-tight flex items-center gap-2"><Target className="w-6 h-6 text-primary dark:text-[#00FF66]" />Matching Engine</h1><p className="text-gray-500 text-sm font-mono">Find the best talent for your project</p></div>

 {/* Input Panel */}
 <div className="bg-card border border-border p-6">
 <h3 className="font-black text-foreground uppercase tracking-tight mb-4">Mission Brief</h3>
 <div className="grid grid-cols-1 gap-4 mb-4 md:grid-cols-[1.5fr,auto]">
 <input
 value={projectName}
 onChange={e => setProjectName(e.target.value)}
 placeholder="Project name..."
 className="w-full bg-transparent border border-border px-4 py-3 text-foreground text-sm outline-none focus:border-primary font-mono transition-colors"
 />
 <button
 onClick={saveProject}
 disabled={savingProject || skills.length === 0}
 className="border border-border text-foreground hover:border-blue-500 uppercase tracking-widest text-xs px-6 py-3 transition-colors disabled:opacity-50"
 >
 ...
 className="w-full bg-transparent border border-border px-4 py-3 text-foreground text-sm placeholder-[#8A8A8A] outline-none focus:border-primary resize-none mb-4 font-mono transition-colors" />

 {/* Skills */}
 <div className="mb-4">
 <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-2 block">Required Skills</label>
 <div className="flex gap-2 mb-2 flex-wrap">
 {skills.map((s, i) => (
 <span key={i} className={`inline-flex items-center gap-1.5 border px-3 py-1 text-[10px] font-mono uppercase tracking-widest ${s.priority === 'Essential' ? 'border-[#9D4EDD]/50 text-[#9D4EDD]' : 'border-[#333333] text-muted-foreground dark:text-[#8a8a8a]'}`}>
 {s.name}
 ...
 className="flex-1 bg-transparent border border-border px-4 py-2.5 text-foreground outline-none focus:border-primary font-mono text-sm transition-colors" />
 <button onClick={() => addSkill(newSkill)} className="border border-[#333333] text-muted-foreground hover:text-gray-900 px-3 py-2.5 transition-colors"><Plus className="w-4 h-4" /></button>
 </div>
 <div className="flex gap-2 mt-2 flex-wrap">
 {SKILL_SUGGESTIONS.filter(s => !skills.find(sk => sk.name === s)).slice(0, 6).map(s => (
 <button key={s} onClick={() => addSkill(s)} className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground border border-border hover:text-gray-900 px-2.5 py-1 transition-colors">{s}</button>
 ))}
 </div>
 </div>
 ...
 <div>
 <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-1 block">Seniority</label>
 <select value={seniority} onChange={e => setSeniority(e.target.value)}
 className="w-full bg-transparent border border-border px-3 py-2.5 text-foreground outline-none focus:border-primary font-mono text-sm transition-colors">
 <option value="">Any</option>{SENIORITY.filter(Boolean).map(s => <option key={s}>{s}</option>)}
 </select>
 </div>
 <div>
 <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-1 block">Budget Ceiling (₹/day)</label>
 <input type="number" value={budget} onChange={e => setBudget(e.target.value)} placeholder="Optional"
 className="w-full bg-transparent border border-border px-3 py-2.5 text-foreground outline-none focus:border-primary font-mono text-sm transition-colors" />
 </div>
 <div>
 <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-1 block">Project Start</label>
 <input type="date" value={projectStart} onChange={e => setProjectStart(e.target.value)}
 className="w-full bg-transparent border border-border px-3 py-2.5 text-foreground outline-none focus:border-primary font-mono text-sm transition-colors" />
 </div>
 <div>
 <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-1 block">Project End</label>
 <input type="date" value={projectEnd} onChange={e => setProjectEnd(e.target.value)}
 className="w-full bg-transparent border border-border px-3 py-2.5 text-foreground outline-none focus:border-primary font-mono text-sm transition-colors" />
 </div>
 <div className="flex items-end pb-1">
 <label className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-muted-foreground cursor-pointer">
 ...
 className="bg-white text-black font-bold uppercase tracking-widest text-xs px-8 py-3 hover:opacity-90 transition-colors disabled:opacity-50 flex items-center gap-2">
 ...
 {projectSaveSuccess && (
 <div className="mt-4 border border-blue-500 bg-[#00FF66]/10 px-4 py-3 text-sm text-primary dark:text-[#00FF66]">
 {projectSaveSuccess}
 </div>
 )}
 </div>
 ...
 <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground whitespace-nowrap">Assign to Project:</label>
 <select
 value={selectedProjectId}
 onChange={e => setSelectedProjectId(e.target.value)}
 className="flex-1 min-w-[200px] bg-transparent border border-border px-3 py-2.5 text-foreground outline-none focus:border-primary font-mono text-sm transition-colors"
 >
 ...
 {assignSuccess && (
 <div className="mt-2 text-sm text-primary bg-[#00FF66]/10 border border-blue-500 px-3 py-2 flex items-center gap-2">
 ...
 <h3 className="text-lg font-black text-foreground uppercase tracking-tight">{results.length} Match{results.length !== 1 ? 'es' : ''} Found</h3>
 ...
 {results.map(m => (
 <div key={m.employee.employeeId} className={`border p-5 bg-card hover:border-[#444] transition-colors ${tierBg(m.confidenceScore)}`}>
 <div className="flex items-start justify-between gap-4">
 <div className="flex-1">
 <div className="flex items-center gap-3 mb-2">
 <span className={`text-3xl font-bold ${tierColor(m.confidenceScore)}`}>{m.confidenceScore}%</span>
 <span className="border border-border text-muted-foreground px-2 py-0.5 text-xs font-mono uppercase tracking-widest">#{m.rank}</span>
 </div>
 <p className="text-gray-900 font-black text-lg">{m.employee.name || 'Unknown Employee'}</p>
 <p className="text-sm text-muted-foreground font-mono">{m.employee.department} • {m.employee.seniorityLevel} • {m.employee.location}</p>
 {m.aiExplanation && <p className="text-sm text-muted-foreground mt-3 bg-card border border-border p-3 italic">{m.aiExplanation}</p>}
 ...
 Projected 90-day savings: <span className={m.roiEstimate.projected90DaySavings > 0 ? 'text-blue-500 font-bold' : 'text-[#FF3333] font-bold'}>
 ...
 <div className={`mt-3 p-3 flex items-center gap-3 border ${m.roiEstimate.costBand === 'below average' ? 'bg-[#00FF66]/10 border-blue-500 dark:border-[#00FF66]/20' : m.roiEstimate.costBand === 'above average' ? 'bg-[#FF3333]/10 border-[#FF3333]/20' : 'bg-white/5 border-border dark:border-white/10'}`}>
 ...
 <div className="text-right space-y-1 text-xs text-muted-foreground font-mono min-w-[120px]">
 ...
 return <span key={i} className={`border px-3 py-1 text-[10px] font-mono uppercase tracking-widest ${isReq ? 'border-[#9D4EDD]/50 text-[#9D4EDD]' : 'border-[#333333] text-muted-foreground dark:text-[#8a8a8a]'}`}>{s.skill_name}</span>;
 })}</div>
 ...
 {/* Feedback + Assign */}
 <div className="flex items-center gap-2 mt-3 pt-3 border-t border-border dark:border-white/10">
 <button onClick={() => sendMatchFeedback('thumbs_up', m)}
 className="text-xs text-muted-foreground hover:text-blue-500 transition-colors font-mono uppercase tracking-widest">👍 Helpful</button>
 <button onClick={() => sendMatchFeedback('thumbs_down', m)}
 className="text-xs text-muted-foreground hover:text-[#FF3333] transition-colors font-mono uppercase tracking-widest">👎 Not helpful</button>
 <div className="flex-1" />
 {assignedIds.has(m.employee.employeeId) ? (
 <span className="text-xs text-primary flex items-center gap-1 font-mono uppercase tracking-widest"><CheckCircle className="w-3.5 h-3.5" /> Assigned</span>
 ) : (
 <button
 onClick={() => assignEmployee(m.employee.employeeId)}
 disabled={assigningId === m.employee.employeeId || !selectedProjectId}
 className="flex items-center gap-1.5 border border-[#333333] text-muted-foreground hover:text-gray-900 uppercase tracking-widest text-xs px-4 py-2.5 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
 ...
 {results.length === 0 && (
 <div className="bg-white/50 border border-border p-12 text-center">
 <Eye className="w-12 h-12 text-muted-foreground mx-auto mb-3" />
 <p className="text-gray-500 font-mono">No matches found. Try broader requirements or different skills.</p>
 </div>
 )}
 </div>
 )}
