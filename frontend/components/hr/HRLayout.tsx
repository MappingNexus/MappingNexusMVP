...
const hrNavItems = [
    ...
    { to: '/hr/projects', icon: BriefcaseBusiness, label: 'PROJECTS' },
            ...
            <header className="h-16 border-b border-gray-200/50 dark:border-white/10 bg-white/70 dark:bg-[#1a1a1c]/70 backdrop-blur-xl flex items-center justify-between px-4 sm:px-6 sticky top-0 z-50 transition-all duration-300">
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="flex items-baseline gap-2 min-w-0">
                        ...
                        <span className="hidden sm:inline text-gray-500 dark:text-[#8A8A8A] text-[10px] font-mono tracking-widest uppercase">| HR console</span>
                    </div>
                </div>
                ...
                <div className="hidden lg:flex items-center gap-2 text-gray-500 dark:text-[#8A8A8A] font-mono text-xs">
                    ...
                    <span>HR OPS LAYER ACTIVE</span>
                </div>

                <div className="flex items-center gap-3 sm:gap-4">
                    <div className="hidden sm:flex items-center gap-2 border border-gray-200 dark:border-white/10 px-3 py-1.5">
                        <Bell className="w-3.5 h-3.5 text-[#FF9900]" />
                        <span className="font-mono text-[10px] uppercase tracking-widest text-gray-500 dark:text-[#8A8A8A]">Live alerts</span>
                    </div>
                    <button
            ...
            <div className="flex flex-1 overflow-hidden relative z-10">
                <aside className="w-72 border-r border-gray-200/50 dark:border-white/10 bg-white/50 dark:bg-[#1a1a1c]/50 backdrop-blur-md hidden md:flex flex-col transition-all duration-300">
                    <div className="py-8 flex-1 flex flex-col overflow-y-auto">
                        <div className="px-6 mb-6">
                            <span className="text-gray-400 dark:text-[#8A8A8A] font-mono text-[10px] tracking-widest uppercase">[ HR NAVIGATION ]</span>
                        </div>
                        {nav}
                    </div>
                ...
                {isSidebarOpen && (
                    <div className="fixed inset-0 z-[60] md:hidden">
                        <button className="absolute inset-0 bg-black/60" onClick={() => setIsSidebarOpen(false)} aria-label="Close HR navigation overlay" />
                        <aside className="absolute left-0 top-0 h-full w-[min(22rem,88vw)] bg-white/95 dark:bg-[#111113]/95 backdrop-blur-xl border-r border-gray-200 dark:border-white/10 flex flex-col">
                            <div className="h-16 flex items-center justify-between px-5 border-b border-gray-200 dark:border-white/10">
                                <span className="font-bold tracking-widest text-xs uppercase">HR Console</span>
                                <button onClick={() => setIsSidebarOpen(false)} className="p-1.5 border border-gray-200 dark:border-white/15 rounded-lg" aria-label="Close HR navigation">
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
